...
void BSP_Init(void)
{
    ...
    //初始化液位传感器
    LIQUID_SENSOR_GPIO_Config();
    
    //初始化雾化器
    ATOMIZER_GPIO_Config();
    
    //初始化液泵
    PUMP_GPIO_Config();
    
    //初始化电磁阀
    SOLENOID_GPIO_Config();
    ...
    /*
        BSP_Init 中只针对引脚进行初始化, 初始化函数中不能含有延时函数
        例如:若这里执行 DS3231_Init/NH3_Sensor_Init 会导致程序挂住
    */
    //外置RTC初始化
    IIC_Init();
    
    //单机版没有 NH3_SENSOR 和 WIFI
    #if NETWORK_VERSION
    //初始化NH3传感器
    NH3_USART_Config();
